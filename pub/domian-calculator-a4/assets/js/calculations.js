(function () {
  ...
  function getBoostedStartingRate(agent) {
    if (agent && agent.startingRate !== undefined && agent.startingRate !== null && agent.startingRate !== '') {
      return positiveNumber(agent.startingRate);
    }
    if (Array.isArray(agent && agent.boostedRates) && agent.boostedRates.length) {
      return positiveNumber(agent.boostedRates[0]);
    }
    if (PAY_SCALES.boostedStartingDefault !== undefined) {
      return positiveNumber(PAY_SCALES.boostedStartingDefault);
    }
    return positiveNumber((PAY_SCALES.boostedDefault || [55])[0]);
  }
  ...
  function getDealRate(agent, dealIndex) {
    ...
    var standardRate = standardRates[standardIndex];

    if (agent.paymentType === 'boosted') {
      return Math.max(standardRate, getBoostedStartingRate(agent) / 100);
    }

    return standardRate;
  }

  function getEffectiveDealRate(config) {
    var source = config || {};
    var fixedRate = percentageOrNull(source.fixedRate);
    var manualDealRate = percentageOrNull(source.manualDealRate);
    var packageFloorRate = percentageOrNull(source.packageFloorRate);
    var contractualFloorRate = percentageOrNull(source.contractualFloorRate);
    var requiredFloor = Math.max(
      packageFloorRate === null ? 0 : packageFloorRate / 100,
      contractualFloorRate === null ? 0 : contractualFloorRate / 100
    );
    var standardRate = Math.max(0, Math.min(1, positiveNumber(source.standardRate)));

    if (fixedRate !== null) {
      return fixedRate / 100;
    }

    if (manualDealRate !== null) {
      return Math.max(manualDealRate / 100, requiredFloor);
    }

    return Math.max(standardRate, requiredFloor);
  }
  ...
  function getRateScaleIndexForDeal(dealCommission, qualifiedDealCount, isQualifiedDeal) {
    ...
    if (isQualifiedDeal || positiveNumber(dealCommission) >= threshold) {
  ...
  function calculateAgent(agent) {
    ...
    for (var i = 0; i < dealCount; i += 1) {
      ...
      var automaticScaleIndex = getRateScaleIndexForDeal(currentDealCommission, qualifiedDealCount, isQualifiedDeposit);
      ...
      if (agent.paymentType === 'fixed') {
        rate = getEffectiveDealRate({
          fixedRate: agent.fixedRate,
          standardRate: PAY_SCALES.standard.partner[0]
        });
      } else if (!isQualifiedDeposit) {
        standardRate = PAY_SCALES.standard.partner[0];
        rate = getEffectiveDealRate({
          standardRate: standardRate,
          packageFloorRate: packageFloorRate,
          contractualFloorRate: contractualFloorRate
        });
        if (rate > standardRate) {
          rateSource = contractualFloorRate !== null
            && contractualFloorRate >= (packageFloorRate === null ? 0 : packageFloorRate)
            ? 'contractualFloor'
            : 'packageFloor';
        } else {
        ...
        standardRate = isTraineeStandard
          ? getTraineeStandardDealRate(scaleIndex)
          : getDealRate(agent, scaleIndex);
        rate = getEffectiveDealRate({
          standardRate: standardRate,
          manualDealRate: row.manualRateOverride,
          packageFloorRate: packageFloorRate,
          contractualFloorRate: contractualFloorRate
        });
        if (row.manualRateOverride !== null) {
          rateSource = rate > row.manualRateOverride / 100 ? 'manualRateWithFloor' : 'manualRate';
    ...
    var careerIntegration = agent.careerIntegration && agent.careerIntegration.effectivePackage
      ? agent.careerIntegration
      : null;
    var motivation = careerIntegration
      ? {
        mode: 'career',
        source: 'career',
        total: positiveNumber(careerIntegration.motivationReserveMonthly),
        monthly: positiveNumber(careerIntegration.motivationReserveMonthly),
        costItems: Array.isArray(careerIntegration.motivationCosts)
          ? careerIntegration.motivationCosts
          : []
      }
      : calculateMotivationReserve(agent);
    var fixedRate = agent.fixedRate;
    ...
    return {
      ...
      startingRate: getBoostedStartingRate(agent),
  ...
  function calculateSchemeVariant(config, variant) {
    var agent = {
      ...
      startingRate: variant.startingRate,
  ...
  function getRetentionScenarioVariants() {
    return [
      { id: 'boosted-55', label: 'Стартовый процент 55%', paymentType: 'boosted', startingRate: 55 },
      { id: 'boosted-70', label: 'Стартовый процент 70%', paymentType: 'boosted', startingRate: 70 },
  ...
  function buildRetentionScenario(agentSource, baselineEconomics, variant) {
    ...
    if (variant.paymentType === 'boosted') {
      scenarioAgent.startingRate = getBoostedStartingRate(variant);
    }
