(function () {
  ...
  function renderAgents() {
    ...
    host.innerHTML = state.agents.map(function (agent, agentIndex) {
      ...
      return '<article class="agent-card" data-agent-id="' + agent.id + '">'
        ...
        + '<label>Как платить агенту<select data-agent-field="paymentType">' + option('standard', 'По стандартной шкале', agent.paymentType) + option('boosted', 'Не ниже выбранного процента', agent.paymentType) + option('fixed', 'Один процент на все сделки', agent.paymentType) + '</select></label>'
        + '<label>Минимальный процент<input type="number" min="0" max="100" step="1" data-agent-field="startingRate" value="' + escapeHtml(agent.startingRate) + '"' + startingDisabled + '></label>'
        + '<label>Один процент на все сделки<input type="number" min="0" max="100" step="1" data-agent-field="fixedRate" value="' + escapeHtml(agent.fixedRate) + '"' + fixedDisabled + '></label>'
  ...
  function renderSummary() {
    ...
    if (block) {
      block.innerHTML = '<h2>Итог по выбранному периоду</h2>'
        ...
        + summaryRow('Мотивации за счёт офиса', money(summary.motivations))
  ...
  function renderAgentSummary(summary) {
    ...
    host.innerHTML = '<div class="agent-summary-cards">' + summary.calculated.map(function (item, index) {
      ...
      return '<article class="agent-summary-card">'
        ...
        + summaryTerm('Остаток офису после основных расходов', money(contribution))
        + '</dl>'
        + '</article>';
    }).join('') + '</div>';
  }
